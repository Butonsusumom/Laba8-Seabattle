...
const translate2=' ММРУП ';
   ...
   procedure isAlive(var t,r,b,l:Boolean;var res:boolean;x,y:Integer;var m:TMAP); { Определяет жив ли корбль после выстрела (остались ли у него еще неубитые клетки)}
   begin
    ...
    procedure obnulay(var m:tmap);{ Проходит весь массив, и если находит убитый корабль, то заменяет все клетки вокруг него на точки}
   ...
   procedure killAShip(var t,r,b,l:Boolean;x,y:Integer;var m:tmap);{ Если корабль убит, то меняет все его клетки с «П» на «У»}
   begin
    m[x][y]:=3;
 ...
 function  Vistrel(var M:TMAp; var count:Integer; x,y:integer):boolean;{ Производит выстрел и соответствующие действия после него( вписывание в клетку соответствующих букв, обнесение убитого корабля точками )}
var
alive1,r,l,t,b:Boolean;
 ...
 procedure ReadFile(path:string; var M:TMAP ); { Считывание файла в двумерный массив, с соответствующим обозначением моря и кораблей}
  var temp: string;
      t1:textfile;
      i,j:integer;
  begin
  ...
  procedure drawField(m:tmap); { Вывод поля активного игрока}
  var tempi,tempj:Integer;
  begin
 ...
 while(tempi<=11) do begin
  ...
  procedure drawenemyField(m:tmap); { Вывод поля пассивного игрока}
  var tempi,tempj:Integer;
  begin
     write('  а б в г д е ж з и к');
     tempi:=2;
 while(tempi<=11) do begin
  ...
  procedure getCoordsletter(var firstco:integer;var secondco:integer); { Проверка на верный ввод хода (координаты выстрела)}
   var letter:String;
   begin
    repeat writeln;
...
begin
  { TODO -oUser -cConsole Main : Insert code here }
   ...
   if(h1st)then Writeln('ход 1 игрока(нажмите ентер чтобы увидеть поле) у вас ',player1ships,' клеток у противника ',player2ships,' клеток')else Writeln('ход 2 игрока(нажмите ентер чтобы увидеть поле) у вас ',player2ships,' клеток у противника ',player1ships,' клеток');
     if (h1st)then begin Readln; drawenemyField(f2);writeln;writeln;drawField(f1) ; end else begin Readln; drawField(f2);writeln;writeln;drawenemyField(f1) end;
